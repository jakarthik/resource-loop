...
async def send_otp(payload: OtpRequest):
    if not valid_email(payload.email): raise HTTPException(400, "Use a NIT Andhra Pradesh institutional email.")
    OTP_CHALLENGES[payload.email.lower()] = {"otp":"123456","created_at":datetime.now(timezone.utc).isoformat()}
    return {"sent":True,"demo_otp":"123456","email":payload.email.lower(),"role":"student" if "@student." in payload.email else "faculty"}

@api.post("/auth/verify-otp")
async def verify_otp(payload: OtpVerify):
    email = payload.email.lower()
    challenge = OTP_CHALLENGES.get(email)
    if not valid_email(email) or not challenge or payload.otp != challenge["otp"]: raise HTTPException(400, "That OTP is not correct or has not been requested for this email.")
    OTP_CHALLENGES.pop(email, None)
    SESSIONS["loop-demo-session"] = DEMO_USER
    return {"token":"loop-demo-session","user":DEMO_USER}

def require_session(authorization: Optional[str] = Header(default=None)):
    token = authorization.replace("Bearer ", "", 1) if authorization and authorization.startswith("Bearer ") else None
    if not token or token not in SESSIONS: raise HTTPException(401, "Authentication required.")
...
app.add_middleware(CORSMiddleware, allow_credentials=False, allow_origins=os.environ["CORS_ORIGINS"].split(","), allow_methods=["*"], allow_headers=["*"])
